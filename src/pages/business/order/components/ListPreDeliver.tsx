...
function ListPreDeliver(props: Props) {
  const { history } = props;

  const { loading, list, paginationConfig } = usePagination<Order.OrderItem>(API.fetchOrderPage, { status: 20 });
...
export default withRouter(ListPreDeliver);
